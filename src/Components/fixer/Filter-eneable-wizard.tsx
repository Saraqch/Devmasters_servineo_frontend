...
export function FixerEnableWizard({ user }: FixerEnableWizardProps) {
  ...
  function validateStep() {
    ...
    if (step === 4) {
      ...
      if (needsAccount && !draft.accountInfo?.trim())
        e.accountInfo = 'Ingrese la cuenta para pagos';
    }
    if (step === 6) {
      if (draft.hasVehicle === null) e.vehicle = 'Debe indicar si cuenta con vehículo';
      if (draft.hasVehicle === true && !draft.vehicleType)
        e.vehicleType = 'Seleccione el tipo de vehículo';
    }
    if (step === 7) {
