...
function AdvancedSearchPage() {
  ...
  // let searchTimeout: ReturnType<typeof setTimeout> | null = null; // Línea duplicada, la he comentado para evitar errores.
  ...
  return (
    <>
      ...
      <main className="pt-20 lg:pt-24 px-4 sm:px-6 md:px-12 lg:px-24 pb-12">
        {/* CAMBIO: Eliminado el mt-4. El pt-20/24 de main ya da suficiente espacio. */}
        <h1 className="text-center text-xl sm:text-2xl md:text-3xl font-bold mb-8">
          Búsqueda Avanzada
        </h1>

        <div className="max-w-7xl mx-auto">
          {/* CAMBIO: Se ajusta el contenedor principal de la búsqueda a un ancho máximo responsivo y se elimina el margen negativo. */}
          <div className="max-w-2xl mx-auto"> 

            <div className="hidden md:block fixed top-28 right-8 z-50 w-72">
              {/* CAMBIO: Se le da al ResultsCounter un ancho completo en móvil y se limita en sm/md. */}
              <div className="w-full sm:w-80">
                <ResultsCounter total={resultsCount ?? 0} loading={loading} />
              </div>
              {/* Versión móvil (dentro del flujo normal) */}
              <div className="block md:hidden w-full mt-4 px-4">
              <ResultsCounter total={resultsCount ?? 0} loading={loading} />
              </div>
            </div>

            {/* CAMBIO: Eliminada la clase problemática w-full sm:w-[700px] mx-auto -mt-60 */}
            <div className="mx-auto mt-4"> 
              ...
              {/* CAMBIO: Se reduce el padding vertical en móvil si es necesario (de py-2 a py-1, y sm:py-2 para mantenerlo en desktop) */}
              <div className="bg-[#2B6AE0] text-white px-4 py-1 sm:py-2 text-sm font-bold mb-6 rounded-lg text-left">
                Parámetros Seleccionables:
              </div>

              {/* ... Filtro de Nombre del fixer ... */}
              <div className="mb-6">
                ...
                {openSections.fixer && (
                  <div className="bg-white border border-t-0 border-gray-300 p-4 rounded-b-lg shadow-sm">
                    {/* El grid ya es responsivo (grid-cols-2 sm:grid-cols-3 md:grid-cols-4) */}
              ...
              <div className="mb-6">
                ...
                {openSections.ciudad && (
                  <div className="bg-white border border-t-0 border-gray-300 p-4 rounded-b-lg shadow-sm max-h-[200px] overflow-y-auto">
                    {/* El grid ya es responsivo (grid-cols-2 sm:grid-cols-3) */}
              ...
              {/* CAMBIO CLAVE: Filtro de Fecha y Calificación - Se usa flex-col en móvil y sm:flex-row en desktop */}
              <div className="mb-6 flex **flex-col** sm:**flex-row** gap-6 items-start">
                <div className="flex-shrink-0 **w-full** sm:**w-auto**"> {/* Asegura que ocupe el 100% en móvil */}
                  <DateFilterSelector />
                </div>
                <div className="flex-shrink-0 **w-full** sm:**w-auto**"> {/* Asegura que ocupe el 100% en móvil */}
